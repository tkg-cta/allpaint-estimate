...
### ステップ2: スクリプトプロパティの設定 (重要)

セキュリティと連携のために、以下の環境変数を設定します。

1. **プロジェクト設定を開く**
   - 左サイドバーの歯車アイコン⚙️「プロジェクトの設定」をクリック

2. **スクリプトプロパティを追加**
   - 画面下部の「スクリプトプロパティ」セクションまでスクロール
   - 「スクリプトプロパティを追加」をクリックして、以下の4つを全て追加してください：

| プロパティ | 説明 | 値の例 |
|-----------|------|--------|
| `LINE_ACCESS_TOKEN` | LINE Messaging APIのアクセストークン | `eyJhbGci...` |
| `LINE_USER_ID` | 通知を送る管理者のLINE UserID | `U123456...` |
| `LIFF_CHANNEL_ID` | LIFFアプリのChannel ID | `1234567890` |
| `SPREADSHEET_ID` | データを保存するスプレッドシートのID | `1A2B3C...` |

> [!TIP]
> **スプレッドシートIDとは？**
> スプレッドシートのURL `https://docs.google.com/spreadsheets/d/abc123xyz/edit` の `abc123xyz` の部分です。
